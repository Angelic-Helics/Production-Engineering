...
- We updated the pipeline to determine the next Docker image version from the latest Git tag and expose both the image version and the full Docker image tag to later pipeline stages.
...
- We updated the deployment step to run the published Docker Hub image together with MongoDB using Docker Compose from Jenkins.
